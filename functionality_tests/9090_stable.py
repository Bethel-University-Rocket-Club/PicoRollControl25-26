...
def write_timepoint(current_time, deltaRoll, cumulativeRoll, motor_power):
    global data_file
    data_file.write(f"{utime.ticks_diff(current_time, LAUNCH_TIME_MS)},{'CCW' if deltaRoll > 0 else 'CW'},{cumulativeRoll},{deltaRoll},{motor_power}\n")
...
def update_apogee(delta_time_s):
    global APOGEE, APOGEE_UPDATE_TIME, gyro, runningVelocity, runningAltitude
    accel_up = gyro.get_accelX() * 9.8
    cur_time = utime.ticks_ms()
    if utime.ticks_diff(cur_time, LAUNCH_TIME_MS) / 1000 > 15:
        return False
    runningVelocity += accel_up * delta_time_s
    runningAltitude += runningVelocity * delta_time_s
    if runningAltitude > APOGEE:
        APOGEE = runningAltitude
        APOGEE_UPDATE_TIME = cur_time
        return True
    elif utime.ticks_diff(cur_time, APOGEE_UPDATE_TIME) / 1000 > 1: #past apogee
        return False
    return True
